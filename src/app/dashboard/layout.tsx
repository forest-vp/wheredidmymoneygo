...
import {
  TrendingDown, LayoutDashboard, PlusCircle, BarChart3, Target,
  Brain, CreditCard, Settings, LogOut, Menu, ChevronRight, Home, Lock,
} from 'lucide-react'
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // Fetch user email from localStorage as fallback
  const displayEmail = user?.email || (typeof window !== 'undefined' ? localStorage.getItem('wdmg_user_email') : null) || ''
  ...
  return (
    <div className="min-h-screen bg-bg flex">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-bg-sidebar border-r border-border flex flex-col transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-6">
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                  isActive ? 'bg-primary/10 text-primary' : locked ? 'text-text-dim/50 cursor-not-allowed' : 'text-text-muted hover:text-text hover:bg-bg-card'
                }`}>
        ...
        <div className="p-3 border-t border-border">
          <div className="px-3 py-2 mb-2">
            ...
            <span className={`inline-block mt-1 text-xs font-medium px-2 py-0.5 rounded-full ${
              userPlan === 'premium' ? 'bg-accent/20 text-accent' : userPlan === 'pro' ? 'bg-primary/20 text-primary' : 'bg-border text-text-dim'
            }`}>
